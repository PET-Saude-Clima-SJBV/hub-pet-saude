...
export interface Coluna { nome: string; obrigatoria?: boolean; descricao: string }
export interface Tabela { cabecalho: string[]; linhas: { numero: number; valores: Record<string, string> }[] }
...
export async function lerTabela(buffer: Buffer, nomeArquivo: string, colunas: Coluna[]): Promise<Tabela> {
  ...
  const bruto = formato === 'xlsx' ? await lerXlsx(buffer) : lerTexto(buffer);
  ...
  const estranhas = cabecalho.filter((c) => c && !conhecidas.has(c));
  if (estranhas.length) throw new BadRequestException(`Coluna desconhecida: ${estranhas.join(', ')}. Use o modelo.`);
